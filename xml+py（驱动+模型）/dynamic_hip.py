...
def control_specific_hip_joint():
    ...
    while True:
        try:
            angle_deg = float(input("请输入向上运动的最大角度 (度): "))
            break
        except ValueError:
            print("错误: 请输入有效的数字")
    ...
    while True:
        try:
            frequency = float(input("请输入往返运动的频率 (Hz): "))
            break
        except ValueError:
            print("错误: 请输入有效的数字")
    
    # 将角度转换为弧度
    max_angle_rad = -np.radians(angle_deg) 
    print(f"\n设置: {joint_name} 将在0到{angle_deg}°之间往返运动")
    print(f"运动频率: {frequency} Hz")
    print("其他三个髋关节将保持不动")
    ...
    print("\n开始模拟...")
    print(f"目标关节: {joint_name} 将在0到{angle_deg}°之间往返运动")
    print(f"运动频率: {frequency} Hz")
    print("其他三个髋关节将保持不动")
    print("非髋关节的腿部关节保持原有正弦运动")
    print("按ESC退出查看器")
        ...
        with mujoco.viewer.launch_passive(model, data) as viewer:
            ...
            while viewer.is_running():
                ...
                for name, jnt_id in joint_ids.items():
                    if jnt_id != -1:
                        if name == joint_name:
                            normalized_pos = (1 - np.cos(2 * np.pi * frequency * simulation_time)) / 2
                            pos = max_angle_rad * normalized_pos
                        elif name in hip_joints:
                            pos = 0.0
                        else:
                            pos = amplitude * np.sin(2 * np.pi * leg_frequency * simulation_time + phases[name])
...
if __name__ == "__main__":
    print("四足机器人指定髋关节往返运动演示")
    print("=" * 50)
    print("您可以指定任意一个髋关节(joint1,6,11,16)在0到指定角度之间往返运动")
    print("其他三个髋关节将保持不动")
    print("非髋关节的腿部关节保持原有正弦运动")
    print("=" * 50)
